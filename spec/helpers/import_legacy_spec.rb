...
file_path = './spec/helpers/PBF_Manager_LIT_2016.csv'

headers = CSV.open(file_path, 'r', &:first)
lines = CSV.read(file_path)
lines.shift
values = lines.each_with_index.map do |entity_line, index|
...
values = values.select { |v| v['District'] == 'Cite des palmiers' }
puts "values : #{values.size}"
puts "quantities: #{quantities.size}"
final_values = []
fosa_names = {}
values.each do |value|
  quantities.each do |quantity|
    index = quantity[:index]
    state_method = (quantity.keys.map(&:to_s) - ['label']).map { |key| [key.to_sym, "#{key}_#{index}"] }.to_h
    dhis2_values = state_method.map { |state, method| [state, value.send(method)] }.map do |v|
      ...
      unless fosa_id
        puts "no matching name #{original_fosa_name} #{fosa_name} #{fosa_id}"

        next
      end
      v = {
        data_element: quantity[state],
        value: current_value,
        org_unit: fosa_id,
        period: period.join(''),
        comment: "legacy-L#{value.line_number}-#{index}-#{period.join('-')}-#{quantity[:label]}-#{original_fosa_name}"
      }
